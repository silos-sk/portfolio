...
export async function getApi (url, data, render) {
  ...
  export function renderPortfolio(data){
    ...
    projCard.innerHTML = `<a href=${data.websiteUrl} target="_blank">
    <picture load="lazy">
        <source
          media="(min-width: 400px)"
          srcset=${data.imgUrl_sm}
        />
        <source
          media="(min-width: 1200px)"
          srcset=${data.imgUrl_lg}
        />
      ...
      <h2>${data.title}</h2>
      <a href=${data.websiteUrl} target="_blank">Website</a> | <a href=${data.gitHubUrl} target="_blank">Github</a>
      <ul>
